...
public class MinimumRectangle {
    public static void main(String[] args) {
        MinimumRectangleSolution minimumRectangleSolution = new MinimumRectangleSolution();
        int[][] sizes = {
                {60, 50},
                {30, 70},
                {60, 30},
        ...
        int solution = minimumRectangleSolution.solution(sizes);
...
class MinimumRectangleSolution {
    public int solution(int[][] sizes) {
        int answer = 0;
        int maxWith = -1;
        int minWith = Integer.MAX_VALUE;
        int maxHeight = -1;
        int minHeight = Integer.MAX_VALUE;
        for(int i = 0; i< sizes.length; i++){
            if(maxWith < sizes[i][0]) {
                maxWith = sizes[i][0];
            }
            if(minHeight > sizes[i][0]){
                minHeight = sizes[i][0];
            }
